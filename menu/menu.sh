...
Exp2=$(curl -sS https://raw.githubusercontent.com/edydevelopeler/D4l4NWeDus/main/ip | grep $MYIP | awk '{print $3}')
export RED='\033[0;31m'
...
function check_and_update() {
    ...
    latest_commit=$(curl -s https://api.github.com/repos/edydevelopeler/EdYPlerJomMoxXZBbanGetv7/commits/main | grep -oP '"sha": "\K[^"]+')
    commit_messages=$(curl -s https://api.github.com/repos/edydevelopeler/EdYPlerJomMoxXZBbanGetv7/commits/main | grep -oP '"message": "\K[^"]+')
    ...
    if [ "$latest_commit" != "$last_commit" ]; then
        ...
        if [[ "$update_choice" == "Y" || "$update_choice" == "y" ]]; then
            ...
            wget -q https://raw.githubusercontent.com/edydevelopeler/EdYPlerJomMoxXZBbanGetv7/main/m-update.sh -O m-update.sh
            chmod +x m-update.sh
            ./m-update.sh
...
mkdir /etc/github
curl -s https://pastebin.com/raw/3rYQ1Urq > /etc/github/api
echo "Khairilsofia2@gmail.com" > /etc/github/email
echo "GeKaStore" > /etc/github/username
m-ip
fi
}
...
clear;clear;clear
echo -e " $COLOR1╭══════════════════════════════════════════════════════════╮${NC}"
echo -e " $COLOR1│${NC} ${COLBG1}                       ${WH}• VPNpremium.id •                    ${NC} $COLOR1$NC│"
echo -e " $COLOR1╰══════════════════════════════════════════════════════════╯${NC}"
echo -e " $COLOR1╭══════════════════════════════════════════════════════════╮${NC}"
...
cd
rm -rf *
wget https://raw.githubusercontent.com/edydevelopeler/EdYPlerJomMoxXZBbanGetv7/main/m-update.sh  >/dev/null 2>&1
clear
